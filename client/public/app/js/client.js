...
let activitySeries = ["Li", "Rb", "K", "Ca", "Na", "Mg", "Al", "Mn", "Zn", "Fe", "Ni", "Sn", "Pb", "H", "Cu", "Ag", "Pt", "Au"]
...
//SVG CONSTRUCTORS
const Line = (ids, center1, center2, color = 'black', thickness = 2, dashed=false , outerId = '#svg') => {
    return $(`<line  
    ${dashed&&'stroke-dasharray="1,1"'}
    class='atom atom-${ids[0]} atom-${ids[1]} atom-bond' x1=${center1.x} y1=${center1.y} x2=${center2.x} y2=${center2.y} style="stroke:${color};stroke-width:${thickness}"></line>`)
}

const Circle = (id, center, color = 'black', radius = 10, outerId = '#svg') => {
    return $(`<circle draggable="true" class='atom atom-${id}' r=${radius} cx=${center.x} cy=${center.y} fill=${color} stroke='black' ondragstart='handleCircleClick("${id}")'></circle>`)
}

const SvgText = (id, center, text, length, color = 'black', size = 12, outerId = '#svg', className) => {
    return $(`<text  draggable="true" ondragstart='handleTextClick("${id}")' style='font-size: ${size}px;' text-anchor="middle" alignment-baseline="central"  class='atom atom-${id} ${className}' x=${center.x} y=${center.y} fill=${color} textLength=${length} >${text}</text>`)
}
...
const Bond = (
    ...
    if (ids[2] == 'triple'){
        ...
        $(outerId).append(Line(ids, center1, center2, '#dddddd', 1, 'dashed'))
    } else {
...
const Atom = (
    ...
    $(outerId).append(SvgText(id, { x: center.x, y: center.y }, text, 14, color.text, 12, outerId, "symbol"))

    if (charge !== 0) {
        $(outerId).append(Circle(id, { x: center.x + 8, y: center.y - 8 }, "white", 5))
        ...
        $(outerId).append(SvgText(id, { x: center.x + 8, y: center.y - 9 }, charge, 8, 'black', 7))
    }
}
...
const render = () => {
    ...
    if (step == states.length - 1  && !chemAnimateConfig.urlNext) $('#nextStep').hide()
    ...
    positions.forEach(position => {
        ...
        if (object.type == 'electron')
            ...
            Atom(position.id, position, object.text, elements[object.text].color, position.charge)
    })
...
const nextStep = () => {
    ...
    states[state.step].positions.forEach((obj) => {
        let obj2 = states[state.step + 1].positions.find(obj2 => obj2.id === obj.id)
        let dx = obj2.x - obj.x
        let dy = obj2.y - obj.y
        if (dx !== 0 || dy !== 0) {
            translation = true
            $('.atom-' + obj.id).css('transform', `translate(${dx}px, ${dy}px)`)
            if (browserOnPhone) {
    ...
    fetch("https://unichord-apps.com/api/chemanimate/animations/exampleAnimations1/" + name)
        ...
        .then(data => {
            ...
            if(data.animation.showActivitySeries){
                $(`#activity-series .list-group`).append(`<p class='text-center text-white mb-0'><b>ACTIVITY SERIES</b></p>`)
                $(`#activity-series .list-group`).append(`<p class='text-center text-white mb-0'>MOST REACTIVE</p>`)

                activitySeries.forEach(element => {
                    $(`#activity-series .list-group`).append(`
                    <li class="list-group-item"><span class='element' style='background:${elements[element] ? elements[element].color.background : 'black'};color:${elements[element] ? elements[element].color.text : 'white'}'> ${element}</span> ${elements[element] ? elements[element].name : null}</li>
                    `)
                });

                $(`#activity-series .list-group`).append(`<p class='text-center text-white mb-0'>LEAST REACTIVE</p>`)
            } 
        })
}
